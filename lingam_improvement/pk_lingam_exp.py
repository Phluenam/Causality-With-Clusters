...
def compute_reachability_violations(matrix, causal_order):
    return compute_violations(get_reachability_matrix(matrix), causal_order)
...
def pairwise_lingam_causal_order(X, prior_knowledge=None):
    model = DirectLiNGAM(prior_knowledge=prior_knowledge, apply_prior_knowledge_softly=True)
    model.fit(X)
    return model.causal_order_

def direct_lingam_causal_order(X, prior_knowledge=None):
    model = DirectLiNGAM(measure='kernel', prior_knowledge=prior_knowledge, apply_prior_knowledge_softly=True)
    model.fit(X)
    return model.causal_order_

def iterative_ica_lingam_causal_order(X, prior_knowledge=None):
    X = X.copy()
    causal_order = []
    num_var = X.shape[1]
    ...
    for i in range(X.shape[1] - 1):
        ...
        cost_B_hat = iter_B_hat.copy()

        if prior_knowledge is not None:
            cost_B_hat[pk == 1] = np.abs(cost_B_hat).sum() + 1.0
            cost_B_hat[pk == 0] = 0

        if X.shape[1] <=20:
            target_index = exact_estimate_causal_order(cost_B_hat)[-1]
        else:
            target_index = cycle_estimate_causal_order(cost_B_hat)[-1]
...
for p in [20]:
    for n in [500]:
        for p_cluster, density_name in [ (0.8, "Dense"), (3 / p, "Sparse")]:
            for pk_frac in [0.0, 0.25, 0.5, 0.75, 1.0]:
                for seed in run_seeds:
                    ...
                    reach = get_reachability_matrix(B_mat)
                    pk = reach.astype(np.int8)
                    np.fill_diagonal(pk, -1)

                    # Hide each off-diagonal element with probability (1 - pk_frac)
                    p_ = pk.shape[0]
                    off_diag_indices = np.array([i * p_ + j for i in range(p_) for j in range(p_) if i != j])

                    n_to_mask = int(len(off_diag_indices) * (1 - pk_frac))
                    mask_indices = np.random.choice(off_diag_indices, n_to_mask, replace=False)
                    pk.flat[mask_indices] = -1

                    # Set diagonal to -1 as required by DirectLiNGAM API
                    np.fill_diagonal(pk, -1)

                    B_hat[pk == 1] = np.abs(B_hat).sum() + 1.0
                    B_hat[pk == 0] = 0

                    for alg, name in [(estimate_causal_order, "Current"), (exact_estimate_causal_order, "Exact"), (cycle_estimate_causal_order, "Greedy"), (iterative_ica_lingam_causal_order, "IterativeICALiNGAM"),  (pairwise_lingam_causal_order, "Direct_pwling")]: # (heuristic_estimate_causal_order, "ELS"),
                        print(name)
                        ...
                        if name in ["Direct_pwling", "Direct_kernel", "IterativeICALiNGAM"]:
                            causal_order = alg(X, prior_knowledge=pk)
                        else:
                        ...
                        violation, _ = compute_violations(B_mat, causal_order)
                        reachability_violation, _ = compute_reachability_violations(B_mat, causal_order)

                        start = time.time()
                        B_guess = estimate_adjacency_matrix(X, causal_order)
                        end = time.time()
                        estimate_time = end - start

                        if name not in ["Direct_pwling", "Direct_kernel"]:
                            time_used += estimate_time

                        B_adj = (B_mat != 0).astype(np.int8)
                        B_guess_adj = (B_guess != 0).astype(np.int8)
                        ...
                        pk_fracs.append(pk_frac)
                        seeds.append(seed)

                        algs.append(name)
                        times.append(time_used)
                        causal_order_times.append(time_used - estimate_time - ica_time)
                        costs.append(cost)
...
df = pd.DataFrame({"p":ps, "n": ns, "Seed": seeds, "Setting": settings, "Prior Knowledge Frac": pk_fracs, "Algorithm": algs, "Time": times, "Causal Order Time": causal_order_times, "Cost": costs, "Causal Order Edge Violations": violations, "Causal Order Reachability Violations": reachability_violations, "SID": sids, "SHD": shds})

df.to_csv(f"{output_dir}/prior_knowledge_lingam_{args.seeds}.csv")
