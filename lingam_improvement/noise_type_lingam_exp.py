        ...
        for p_cluster, density_name in [ (0.8, "Dense"), (3 / p, "Sparse")]:
            for noise_type in ["normal_exp", "uniform", "triangular", "laplace", "exponential"]:
                for seed in run_seeds:
                    print(p, seed, n, noise_type)
                    np.random.seed(seed)

                    q = p
                    m = 1

                    X, B_mat, B_hat, ica_time = get_data(q, m, n, p_cluster, p_var_within_cluster, p_var_between_cluster, seed, noise_type=noise_type)

                    for alg, name in [(estimate_causal_order, "Current"), (exact_estimate_causal_order, "Exact"), (cycle_estimate_causal_order, "Greedy"), (iterative_ica_lingam_causal_order, "IterativeICALiNGAM"), (heuristic_estimate_causal_order, "ELS"), (pairwise_lingam_causal_order, "Direct_pwling"), (direct_lingam_causal_order, "Direct_kernel")]: # (heuristic_estimate_causal_order, "ELS"),
                        print(name)
...
df = pd.DataFrame({"p":ps, "n": ns, "Seed": seeds, "Setting": settings, "Noise Type": noise_types, "Algorithm": algs, "Time": times, "Causal Order Time": causal_order_times, "Cost": costs, "Causal Order Edge Violations": violations, "Causal Order Reachability Violations": reachability_violations, "SID": sids, "SHD": shds})

df.to_csv(f"{output_dir}/noise_type_lingam_exp_{args.seeds}.csv")
